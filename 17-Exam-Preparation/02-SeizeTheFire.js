function seizeTheFire(array) {
  ...
  for (let i = 0; i < fireCells.length; i++) {
    ...
    if (typeOfFire === 'High' && valueOfFire >= 81 && valueOfFire <= 125) {
      if (currentWater >= valueOfFire) {
        currentWater -= valueOfFire;
        cells.push(valueOfFire);
        effort += (valueOfFire * 0.25);
        totalFire += valueOfFire;
      }
    } else if (typeOfFire === 'Medium' && valueOfFire >= 51 && valueOfFire <= 80) {
      if (currentWater >= valueOfFire) {
        currentWater -= valueOfFire;
        cells.push(valueOfFire);
        effort += (valueOfFire * 0.25);
    ...
    } else if (typeOfFire === 'Low' && valueOfFire >= 1 && valueOfFire <= 50) {
      if (currentWater >= valueOfFire) {
        currentWater -= valueOfFire;
        cells.push(valueOfFire);
        effort += (valueOfFire * 0.25);
